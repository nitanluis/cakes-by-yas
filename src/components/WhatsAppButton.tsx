...
export default function WhatsAppButton() {
  ...
  return (
    <div ref={containerRef} className="fixed bottom-6 right-6 lg:bottom-8 lg:right-8 z-50 flex flex-col items-end">
      ...
      <div 
        ref={menuRef}
        className="mb-4 flex flex-col gap-1 p-2 bg-[var(--color-bg-card)]/85 backdrop-blur-2xl border border-[var(--color-border)] rounded-md shadow-image opacity-0 pointer-events-none scale-95 translate-y-4"
        style={{ transformOrigin: "bottom right" }}
      >
        <a href={WA_URL} target="_blank" rel="noopener noreferrer" className="contact-item flex items-center gap-4 px-5 py-3.5 rounded-md hover:bg-[var(--color-bg)] transition-colors group">
        ...
        <a href={IG_URL} target="_blank" rel="noopener noreferrer" className="contact-item flex items-center gap-4 px-5 py-3.5 rounded-md hover:bg-[var(--color-bg)] transition-colors group">
        ...
        <a 
          ...
          onClick={handleSMS}
          className="contact-item flex items-center gap-4 px-5 py-3.5 rounded-md hover:bg-[var(--color-bg)] transition-colors group"
        >
      ...
      <button
        ...
        className={`px-6 py-3.5 md:py-4 rounded-md flex items-center gap-3 transition-all duration-500 relative overflow-hidden group hover:scale-[1.03] active:scale-95 ${!isOpen ? 'shadow-[0_8px_25px_rgba(234,103,125,0.4)] hover:shadow-[0_12px_30px_rgba(234,103,125,0.6)]' : 'shadow-lg'}`}
        style={{ 
